...
Address = 'catml@202.120.101.188';  %Your target supercomputer: username@address.
Path = [Home '/' USPEX '/CalcFold' num2str(Folder)]; %just keep it

%Step2: Check JobID, the exact command to check job by jobID
[a,b]=unix(['ssh  ' Address ' /usr/bin/qstat ' num2str(jobID)])
...
%     [nothing, nothing] = unix(['scp -i ~/.ssh/seawulf ' Address ':' Path '/OUTCAR ./']) %OUTCAR is not necessary by default
      [nothing, nothing] = unix(['scp  ' Address ':' Path '/OSZICAR ./']) %For reading enthalpy/energy
      [nothing, nothing] = unix(['scp  ' Address ':' Path '/CONTCAR ./']) %For reading structural info
    else
       doneOr = 0;
    end
